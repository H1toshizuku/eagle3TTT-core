...
class Scheduler(
    ...
    def __init__(
        ...
        self.cumulative_alpha_sum = 0.0
        self.cumulative_req_count = 0
        self.last_alpha = 0.0
    ...
    def _log_req_metrics(self, req: Req):
        """Log alpha metrics for a finished request."""
        if not self.server_args.enable_online_ttt:
            return
        
        # === 关键修改：WandB 在子进程中的惰性初始化 ===
        # 因为 Scheduler 是独立进程，主进程的 wandb.init 对它不可见
        if wandb.run is None:
            try:
                # 尝试连接到同一个 Project
                # group="scheduler" 让它在 WandB UI 里和 worker 区分开
                wandb.init(
                    project=os.environ.get("WANDB_PROJECT", "eagle3-ttt-gsm8k"),
                    group="scheduler",
                    name=f"scheduler-pid{os.getpid()}",
                    job_type="metrics-logger",
                    # 设置 resume=True 或 reinit=True 防止冲突
                    reinit=True
                )
            except Exception as e:
                # 如果初始化失败（比如网络问题），只打印一次日志，防止刷屏
                if self.cumulative_req_count % 10 == 0:
                    logger.info(f"[Metrics] WandB init failed in Scheduler: {e}")
        # ============================================

        # 避免重复记录
        if getattr(req, "_logged_to_wandb", False):
            return

        # 获取 EAGLE Worker 统计的数据
        accepted_tokens = getattr(req, "spec_accepted_tokens", 0)
        verify_ct = getattr(req, "spec_verify_ct", 0)
        k = getattr(self.server_args, "speculative_num_draft_tokens", 0) or 0

        # 计算 Instant Alpha (当前请求的接受率)
        alpha = 0.0
        if verify_ct > 0 and k > 0:
            alpha = accepted_tokens / (verify_ct * k)

        # 更新累计 Alpha
        self.cumulative_alpha_sum += alpha
        self.cumulative_req_count += 1
        avg_alpha = self.cumulative_alpha_sum / self.cumulative_req_count
        self.last_alpha = alpha

        # 构造日志数据
        log_dict = {
            "inference/instant_alpha": alpha,
            "inference/cumulative_alpha": avg_alpha,
            "inference/req_len": len(req.output_ids),
            "inference/num_verifies": verify_ct,
            "inference/accepted_tokens": accepted_tokens,
            "inference/finished_req_count": self.cumulative_req_count
        }
        ...
        if wandb.run is not None:
            ...
            # 如果 WandB 真的挂了，回退到 logger
            if self.cumulative_req_count % 10 == 0:
                logger.info(f"[Metrics] {log_dict}")
        
        # 标记该请求已记录
        req._logged_to_wandb = True
