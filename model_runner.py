...
from sglang.srt.speculative.online_structs import OnlineExperienceBuffer
from sglang.srt.speculative.shadow_trainer import ShadowTrainerProcess
...
class ModelRunner:
    ...
    def initialize(self, min_per_gpu_memory: float):
        ...
        # Initialize Online TTT if enabled
    ...
    def _init_online_ttt(self):
        ...
        # 简单处理：仅在 TP=0, PP=0 上启动，避免多进程重复训练
        ...
        # 如果是单卡且设备相同，显存压力会很大，打个日志提醒
        if train_device == f"cuda:{self.gpu_id}":
            logger.info(f"[Online TTT] Shared GPU mode: Inference and Training both on {train_device}")
        else:
            logger.info(f"[Online TTT] Dual GPU mode: Inference on cuda:{self.gpu_id}, Training on {train_device}")
        
        # 1. WandB Init (for worker process)
        if not wandb.run:
            run_name = f"worker-gpu{self.gpu_id}-pid{os.getpid()}"
            wandb.init(
                project=os.environ.get("WANDB_PROJECT", "eagle3-ttt-gsm8k"),
                group="worker",
                name=run_name,
                job_type="shadow-trainer"
            )
        ...
        # 3. Extract Weights for Shadow Trainer
        # Draft Weights (CPU)
        draft_weights = {k: v.cpu() for k, v in self.model.state_dict().items()}
        
        # Target Head Weights (CPU)
        target_head_weights = self.model.lm_head.weight.detach().cpu()
        
        # 4. IPC Queues
        self.shadow_data_queue = mp.Queue()
        self.shadow_result_queue = mp.Queue()
        
        # 5. Start Shadow Process
    ...
    def check_and_sync_ttt(self):
        """
        Called by EAGLEWorker after collecting data.
        Checks if update interval is reached, triggers training, and syncs weights.
        """
        if (
            not self.is_draft_worker 
            or not self.server_args.enable_online_ttt
            or not hasattr(self, "ttt_buffer")
        ):
            return
        ...
        logger.info(f"[Online TTT] Triggering update after {self.ttt_request_count} requests.")
            
        # 1. Get Data and Clear Buffer
        experiences = self.ttt_buffer.get_all_and_clear()
        self.ttt_request_count = 0  # Reset counter immediately
        
        if not experiences:
            logger.info("[Online TTT] Buffer empty, skipping update.")
            return

        # 2. Send to Shadow Trainer
        try:
            self.shadow_data_queue.put(("TRAIN_AND_SYNC", experiences))
        except Exception as e:
            logger.error(f"[Online TTT] Failed to send data: {e}")
            return
            
        # 3. Block and Wait for Result
        try:
            result = self.shadow_result_queue.get()
        except Exception as e:
            logger.error(f"[Online TTT] Failed to receive result: {e}")
            return
            
        # Unpack result (weights, loss)
        if isinstance(result, tuple):
            new_weights, avg_loss = result
        else:
            new_weights, avg_loss = result, None
            
        # Log Loss
        if avg_loss is not None and wandb.run is not None:
            self.ttt_update_step += 1
            wandb.log({"ttt/distill_loss": avg_loss, "ttt/step": self.ttt_update_step})
            
        # 4. Update Weights
        if new_weights:
            missing, unexpected = self.model.load_state_dict(new_weights, strict=False)
            if len(missing) > 0:
                # Filter out expected missing keys (like lm_head) to reduce log noise
                real_missing = [k for k in missing if "lm_head" not in k]
                if real_missing:
                    logger.debug(f"[Online TTT] Weights updated. Missing keys: {real_missing}")
