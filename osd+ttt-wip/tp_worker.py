...
class TpModelWorker(BaseTpWorker):
    ...
    def forward_batch_generation(
        ...
        if model_worker_batch is not None:
            ...
            # [Online Distillation] Force hidden state capture if needed
            if is_verify and getattr(self.model_runner.server_args, "enable_online_distill", False):
        ...
        if self.pp_group.is_last_rank:
            ...
            # [Online Distillation] Extract teacher info
            if is_verify and getattr(self.model_runner.server_args, "enable_online_distill", False):
                 # Extract Teacher Logits (Soft Label)
                 if logits_output.next_token_logits is not None:
                     batch_result.teacher_logits = logits_output.next_token_logits.detach().clone()

                 # Extract Teacher Hidden States (EAGLE Input)
                 if logits_output.hidden_states is not None:
                     batch_result.teacher_hidden_states = logits_output.hidden_states.detach().clone()
                 else:
                     logger.warning("Online Distill enabled but hidden_states is None. Check CaptureHiddenMode setting.")
