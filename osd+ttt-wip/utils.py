...
class GenerationBatchResult:
    ...
    # --- Online Distillation Fields ---
    # These fields are populated only when enable_online_distill is True and is_verify is True.
    # Used to transport teacher knowledge from Target Worker back to Eagle Worker.
    teacher_logits: Optional[torch.Tensor] = None
    teacher_hidden_states: Optional[torch.Tensor] = None
    draft_token_indices: Optional[torch.Tensor] = None
    ...
    def copy_to_cpu(self, return_logprob: bool = False):
        ...
        # Note: teacher_logits and teacher_hidden_states are typically consumed 
        # on GPU by the Eagle Trainer (Shadow Model), so we generally do NOT 
        # need to copy them to CPU here unless we want to offload training to CPU (unlikely).
    ...
    def from_pp_proxy(
        ...
        return cls(
            ...
            # PP (Pipeline Parallel) support for distillation is complex and deferred.
            # We leave teacher_logits/hidden_states as None here.
        )
