...
def safe_copy(src_mod, dst_mod, name_hint=""):
    """Helper to safely copy weights between modules."""
    ...
    if src_mod.weight.data.shape != dst_mod.weight.data.shape:
        logger.warning(f"Shape mismatch in {name_hint}: src {src_mod.weight.shape} vs dst {dst_mod.weight.shape}")
        return
...
def sync_shadow_to_inference_weights(shadow_model, inference_model, tp_rank, tp_size):
    """
    Sync weights from Shadow Model (Full Weights, Single Device)
    to Inference Model (Sharded/Merged Weights, TP).
    Targeting Option 3: Full Draft Head Sync (FC + MidLayer + Norm).
    """
    # 1. FC Layer
    # Shadow: model.model.fc -> Inference: model.fc
    if hasattr(shadow_model, "model") and hasattr(shadow_model.model, "fc") and hasattr(inference_model.model, "fc"):
        safe_copy(shadow_model.model.fc, inference_model.model.fc, "fc")

    # 2. Decoder Layer (Single Layer for Eagle3)
    # Shadow: model.model.midlayer -> Inference: model.layers[0]
    shadow_layer = shadow_model.model.midlayer
    inf_layer = inference_model.model.layers[0]

    # 2.1 QKV Proj (Merged)
    if hasattr(shadow_layer.self_attn, "qkv_proj") and hasattr(inf_layer.self_attn, "qkv_proj"):
        # Assuming TP=1 as per requirement, direct copy
        safe_copy(shadow_layer.self_attn.qkv_proj, inf_layer.self_attn.qkv_proj, "qkv_proj")

    # 2.2 O Proj
    if hasattr(shadow_layer.self_attn, "o_proj") and hasattr(inf_layer.self_attn, "o_proj"):
        safe_copy(shadow_layer.self_attn.o_proj, inf_layer.self_attn.o_proj, "o_proj")

    # 2.3 Gate Up Proj (Merged)
    if hasattr(shadow_layer.mlp, "gate_up_proj") and hasattr(inf_layer.mlp, "gate_up_proj"):
        safe_copy(shadow_layer.mlp.gate_up_proj, inf_layer.mlp.gate_up_proj, "gate_up_proj")

    # 2.4 Down Proj
    if hasattr(shadow_layer.mlp, "down_proj") and hasattr(inf_layer.mlp, "down_proj"):
        safe_copy(shadow_layer.mlp.down_proj, inf_layer.mlp.down_proj, "down_proj")

    # 2.5 Norms
    safe_copy(shadow_layer.input_layernorm, inf_layer.input_layernorm, "input_layernorm")
    safe_copy(shadow_layer.post_attention_layernorm, inf_layer.post_attention_layernorm, "post_attention_layernorm")
    if hasattr(shadow_layer, "hidden_norm") and hasattr(inf_layer, "hidden_norm"):
        safe_copy(shadow_layer.hidden_norm, inf_layer.hidden_norm, "hidden_norm")

    # 3. Final Norm
    if hasattr(shadow_model.model, "norm") and hasattr(inference_model.model, "norm"):
        safe_copy(shadow_model.model.norm, inference_model.model.norm, "final_norm")


@torch.no_grad()
def sync_inference_to_shadow_weights(inference_model, shadow_model, tp_rank, tp_size):
    """
    Sync weights from Inference Model to Shadow Model.
    Used for Initialization.
    """
    # 1. FC Layer
    if hasattr(shadow_model, "model") and hasattr(shadow_model.model, "fc") and hasattr(inference_model.model, "fc"):
        safe_copy(inference_model.model.fc, shadow_model.model.fc, "fc_init")
    ...
    # 2.1 QKV Proj
    if hasattr(shadow_layer.self_attn, "qkv_proj") and hasattr(inf_layer.self_attn, "qkv_proj"):
        safe_copy(inf_layer.self_attn.qkv_proj, shadow_layer.self_attn.qkv_proj, "qkv_proj_init")

    # 2.2 O Proj
    if hasattr(shadow_layer.self_attn, "o_proj") and hasattr(inf_layer.self_attn, "o_proj"):
        safe_copy(inf_layer.self_attn.o_proj, shadow_layer.self_attn.o_proj, "o_proj_init")

    # 2.3 Gate Up Proj
    if hasattr(shadow_layer.mlp, "gate_up_proj") and hasattr(inf_layer.mlp, "gate_up_proj"):
        safe_copy(inf_layer.mlp.gate_up_proj, shadow_layer.mlp.gate_up_proj, "gate_up_proj_init")

    # 2.4 Down Proj
    if hasattr(shadow_layer.mlp, "down_proj") and hasattr(inf_layer.mlp, "down_proj"):
        safe_copy(inf_layer.mlp.down_proj, shadow_layer.mlp.down_proj, "down_proj_init")

    # 2.5 Norms
    safe_copy(inf_layer.input_layernorm, shadow_layer.input_layernorm, "input_layernorm_init")
    safe_copy(inf_layer.post_attention_layernorm, shadow_layer.post_attention_layernorm,
              "post_attention_layernorm_init")
    if hasattr(inf_layer, "hidden_norm") and hasattr(shadow_layer, "hidden_norm"):
        safe_copy(inf_layer.hidden_norm, shadow_layer.hidden_norm, "hidden_norm_init")

    # 3. Final Norm
    if hasattr(shadow_model.model, "norm") and hasattr(inference_model.model, "norm"):
        safe_copy(inference_model.model.norm, shadow_model.model.norm, "final_norm_init")